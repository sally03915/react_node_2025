...
function* follow(action) {
    try {
        ...
        yield put({
            type: FOLLOW_SUCCESS,
            data: action.data,   // 팔로우할 사람의 아이디르 넘겨받음
        });
    } catch (err) {
        console.error(err);
        yield put({
...
function* unfollow(action) {
    try {
        ...
        yield put({
            ...
            data: action.data,  // 언팔로우할 사람의 아이디르 넘겨받음
        });
    } catch (err) {
        console.error(err);
        yield put({
...
function* watchFollow() { //##3
    yield takeLatest(FOLLOW_REQUEST, follow);
}

function* watchUnfollow() { //##4
